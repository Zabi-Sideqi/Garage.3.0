...
public static class DbInitializer
{
    public static async Task InitializeAsync(IServiceProvider services)
    {
        ...
        // Database migrations are handled by CI/CD (GitHub Actions).

        // 1. Roles & Admin
        // Must run first for OwnerId dependencies.
        await SeedRolesAndAdminAsync(services);

        // 2. Vehicle Types & Parking Spots
        // Must run before Vehicles for Type FKs.
    ...
    private static async Task SeedVehicleTypesAndParkingSpotsAsync(
        ...
        IServiceProvider services)
    {
        if (!await context.VehicleTypes.AnyAsync())
        {
            var vehicleTypes = new[]
            {
                ...
                // of 3. Car takes a whole spot; Motorcycle/Bicycle share one
                // (3 fit); large vehicles need more than one spot's capacity (US12).
                ...
                new VehicleTypeEntity
                {
                    ...
                    RequiredSpots = 1,
                    MaxVehiclesPerSpot = 3,
                    RequiredSpaceUnits = 1
                },
        ...
        {
            var settings =
                services.GetRequiredService<IOptions<GarageSettings>>().Value;

            var spots = Enumerable
                .Range(1, settings.TotalParkingSpots)
        ...
        IServiceProvider services)
    {
        ...
        var adminUser =
            await userManager.FindByEmailAsync("admin@garage.com");
        ...
        var typesByName =
            await context.VehicleTypes
                .ToDictionaryAsync(vt => vt.Name, vt => vt.Id);

        var vehiclesToSeed = new List<Vehicle>
        {
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Car"],
            ...
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Motorcycle"],
                OwnerId = adminUser.Id,
            ...
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Bus"],
                OwnerId = adminUser.Id,
            ...
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Truck"],
                OwnerId = adminUser.Id,
            ...
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Bicycle"],
                OwnerId = adminUser.Id,
            ...
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Airplane"],
                OwnerId = adminUser.Id,
            ...
            new Vehicle
            {
                VehicleTypeRefId = typesByName["Boat"],
                OwnerId = adminUser.Id,
        ...
        // Seeded vehicles are registered only, not auto-parked,
        // so there is something to try out manually on the
        // "Park a Vehicle" page.
    }
}